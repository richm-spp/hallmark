...
B1. **Brand accent vs paper contrast.** With a brand locked, does `--color-accent-ink` reach **4.5:1** against `--color-paper`, do `--color-accent` and `--color-focus` reach **3:1** (WCAG 1.4.11), and does `--color-on-accent` reach **4.5:1 against `--color-accent`** (the on-fill pairing — accent-ink and on-accent are different tokens, and setting them equal ships invisible button labels)? On dark-paper themes, are the brand file's `[data-paper="dark"]` values in use rather than the light-paper ones? And does any token the brand file marks decorative-only (e.g. `--brand-plus`, wordmark glyph) appear in a `color:` declaration outside the wordmark? Any yes → fail. Compute the ratios — do not eyeball them.
